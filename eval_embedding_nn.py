...
def eval_once(data_loader, saver, placeholders, recall):
    with tf.Session() as sess:
        ...
        vd_feats, sent_feats, labels = data_loader.get_batch(0, FLAGS.batch_size, FLAGS.sample_size)
        feed_dict = {
                placeholders['vd_feat'] : vd_feats,
...
def main(_):
    # Load data.
    data_loader = DatasetLoader(FLAGS.video_feat_path, FLAGS.sent_feat_path, split='eval')
    num_vds, vd_seq_len, vd_feat_dim = data_loader.vd_feat_shape
    num_sents, sent_feat_dim = data_loader.sent_feat_shape

    # Setup placeholders for input variables.
    vd_feat_plh = tf.placeholder(tf.float32, shape=[FLAGS.batch_size, vd_seq_len, vd_feat_dim])
    ...
    recall = setup_eval_model(vd_feat_plh, sent_feat_plh, train_phase_plh, label_plh)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_feat_path', type=str, help='Path to the video feature mat file.')
    ...
    # Training parameters.
    parser.add_argument('--batch_size', type=int, default=425, help='Batch size for evaluation.')
    parser.add_argument('--sample_size', type=int, default=17, help='Number of positive pair to sample.')
