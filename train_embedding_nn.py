...
def main(_):
    # Load data.
    data_loader = DatasetLoader(FLAGS.video_feat_path, FLAGS.sent_feat_path)
    ...
    loss = setup_train_model(vd_feat_plh, sent_feat_plh, train_phase_plh, label_plh, FLAGS)
    ...
    with tf.device('/gpu:0'):
        with tf.Session() as sess:
            ...
            for i in range(num_steps):
                ...
                vd_feats, sent_feats, labels = data_loader.get_batch(
                        i % steps_per_epoch, FLAGS.batch_size, FLAGS.sample_size)
                feed_dict = {
                        vd_feat_plh : vd_feats,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--im_loss_factor', type=float, default=1.5,
                        help='Factor multiplied with video loss. Set to 0 for single direction.')
